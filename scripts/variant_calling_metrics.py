...
def main(options):
    ...
    reads_mapped_with_min_mapq_diff_aln_len = 0
    reads_mapped_diff_aln_len = 0
	...
	if temp_line1[0].strip() == temp_line2[0].strip():
		...
		if not diff_field( rname_sam1, rname_sam2 ): #if chr are not equal
		    ...
		    if not diff_field(sam1_aln_len, sam2_aln_len):
			reads_mapped_diff_aln_len += 1
		...
		if mapq_sam1 > options.min_mapq and mapq_sam2 > options.min_mapq: #if mapq > options.mapq
			...
			(cigar_sam1_MD, count1) = re.subn(r'[MD]', '|', re.subn(r'\d+[SIHNP]', '', cigar_sam1)[0])
                        ...
                        (sam1_aln_len, sam2_aln_len) = aln_len(cigar_sam1_MD, cigar_sam2_MD)
                        if not diff_field(sam1_aln_len, sam2_aln_len):
                            reads_mapped_with_min_mapq_diff_aln_len += 1
    ...
    print "With different alignment lengths = " + str(reads_mapped_diff_aln_len)
    ...
    print "At different alignment positions = " +  str(reads_mapped_with_min_mapq_diff_aln_pos)
    print "With different alignment lengths = " + str(reads_mapped_with_min_mapq_diff_aln_len)
    print "With different cigar strings = " + str(reads_mapped_with_min_mapq_diff_cigar_string)
