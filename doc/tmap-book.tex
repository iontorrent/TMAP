...
Note that that ambiguous IUPAC codes will be randomly converted to a DNA base that does not match any of the bases represented by the IUPAC code to ensure minimum reference bias.
...
\subsubsection{RG}
This stores the read group identifier corresponding to the RG SAM header field.

\subsubsection{PG}
This stores the program group identifier corresponding to the PG SAM header field.

\subsubsection{MD}
This stores the MD array.
The goal of this array is to follow the conventions in SAMtools (\cite{SAM-format}).
Any IUPAC reference code will be present in this field.

\subsubsection{NM}
This stores the edit distance from the reference sequence.
Matches to any non-DNA and non-N IUPAC ambiguity code (B,D, H, K, M, R, S, V, W, Y) will be counted as a mismatch.
Since all Ns are mutated to As in the packed reference, A-N mismatches will not be counted.

\subsubsection{AS}
This stores the alignment score.
If the alignment contains an N in the reference but an A in the read, this score will be slightly reduced.
