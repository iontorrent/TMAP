                         ...
                         int32_t tid, fmap_map_all_opt_t *opt)
{
  ...
  for(i=0;i<2;i++) {
      ...
      if(opt->algos[i] & FMAP_MAP_ALL_ALGO_MAP3) {
          ...
          if(NULL == flow_map3[0] && NULL == flow_map3[1]) {
              flow_map3[0] = fmap_malloc(sizeof(uint8_t) * 4, "flow_map3[0]");
              flow_map3[1] = fmap_malloc(sizeof(uint8_t) * 4, "flow_map3[1]");
              for(j=0;j<4;j++) {
                  flow_map3[0][j] = opt->opt_map3[i]->flow[j];
                  flow_map3[1][j] = 3 - opt->opt_map3[i]->flow[j];
              }
          }
      }
  }
  ...
  while(low < seq_buffer_length) {
      ...
      while(low<high) {
          ...
          for(i=0;i<2;i++) {
              ...
              // fmap_map3_aux_core
              if(opt->algos[i] & FMAP_MAP_ALL_ALGO_MAP3) {
                  aln_map3 = fmap_map3_aux_core(seq, flow_map3, refseq, bwt[1], sa[1], opt->opt_map3[i]);
              }
              else {
                  // empty
