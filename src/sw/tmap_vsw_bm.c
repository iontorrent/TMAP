...
static int
usage(int32_t seq_len, int32_t tlen, int32_t n_iter, int32_t use_sw)
{
...
tmap_vswbm_main(int argc, char *argv[])
{
  ...
  while((c = getopt(argc, argv, "q:t:n:Sh")) >= 0) {
      switch(c) {
        case 'q':
        ...
        case 'h':
        default:
          return usage(seq_len, tlen, n_iter, use_sw);
      }
  }
  if(argc != optind) {
      return usage(seq_len, tlen, n_iter, use_sw);
  ...
  tmap_vsw_bm_core(seq_len, tlen, n_iter, use_sw);
